...
const allTargetsSchema = z.object({
  [SupportedLanguage.Node]: nodeOrReactLanguageSpecificSchema,
  [SupportedLanguage.React]: nodeOrReactLanguageSpecificSchema,
})
...
const DEFAULT_CONFIG: {
  ...
  targets: {
    'node-ts': {
      ...
      declarationFileName: 'reforge-server.d.ts',
    },
    'react-ts': {
      clientFileName: 'reforge-client.ts',
      declarationFileName: 'reforge-client.d.ts',
    },
  },
}
      ...
      for (const target of targets) {
        ...
        if ([SupportedLanguage.Node, SupportedLanguage.React].includes(language)) {
          ...
          fileCreationPromises.push(
            fileManager.writeFile({data: generatedCode, filename: targetConfig.declarationFileName}),
          )
        }
      }
